...
def cosine_sim(data_without_target,windows,num_windows):
    ...
    """
    sim = np.zeros((data_without_target.shape[1],data_without_target.shape[1],num_windows))
    for i in range(num_windows):
        segmented_window=data_without_target.iloc[windows[i]['start_index']:windows[i]['end_index']+1,:].T.to_numpy()
...
def page_rank_to_feature(pr):
    ...
    sum_list=[np.sum(pr_reshaped[:,i])+np.sum(pr_reshaped[i,:]) for i in range(pr_reshaped.shape[0])]
    sum_list_indice=sorted(range(len(sum_list)), key=lambda k: sum_list[k] ,reverse=True)
...
def featureSelection(data_without_target,window_size):
    ...
    feature_similarity_tensor=cosine_sim(data_without_target,win_index_dic,num_windows) # sim
